		...
		while (alternador) {
			...
			case 3: //remover
				...
				try {
					...
					System.out.println("A lista esta vazia ou não tem elemento nessa posição");
				}
			...
			case 5:
				...
				for(int i= 0; i<arranjo.size();i++) {
					if(arranjo.get(i).equals(procurar)) {
						posicao = i;
						achado = true;
					}
				}
				...
				if(achado == true) {
					System.out.println("O número " + procurar + " está na posição " + (posicao)+ " no arranjo");
